...
// Lane assignment with sorting - earliest lane start goes to top
function assignLanesSorted(tasks) {
  // 1) Sort tasks by start date (nulls last)
  const sorted = [...tasks].sort((a, b) => {
    if (!a.start && !b.start) return 0;
    if (!a.start) return 1;
    if (!b.start) return -1;
    const d = new Date(a.start) - new Date(b.start);
    return d !== 0 ? d : ((new Date(a.end || a.start)) - (new Date(b.end || b.start)));
  });

  // 2) Greedy lane assignment - STRICT no overlaps within lane
  const lanesEnd = [];     // end of last bar in lane
  const lanesStart = [];   // earliest start of lane
  const laneOf = {};
  ...
  for (const t of sorted) {
    ...
    // Handle tasks without dates - put them in lane 0
    if (!s || !e) {
      laneOf[t.id] = 0;
      if (lanesEnd.length === 0) {
        lanesEnd.push(new Date(0));
        lanesStart.push(new Date(8640000000000000));
      }
      continue;
    }

    let placed = false;
    // Try to find a lane where this task doesn't overlap
    for (let i = 0; i < lanesEnd.length; i++) {
      // STRICT: task can start only if previous task in lane has ended
      // Use < instead of <= to prevent same-time overlaps
      if (lanesEnd[i] < s) {
        lanesEnd[i] = e;
        laneOf[t.id] = i;
        // Update lane start if this task starts earlier
        if (s < lanesStart[i]) {
          lanesStart[i] = s;
        }
        placed = true;
  ...
  // 3) Sort lanes by earliest start - earliest lane goes to top (index 0)
  const order = lanesEnd.map((_, i) => i).sort((i, j) => {
    const si = lanesStart[i] ?? new Date(8640000000000000);
    const sj = lanesStart[j] ?? new Date(8640000000000000);
    return si - sj;
  });

  // 4) Remap lane indices to order by earliest start
  const remap = new Map();
  order.forEach((laneIdx, newIdx) => remap.set(laneIdx, newIdx));
  Object.keys(laneOf).forEach(id => laneOf[id] = remap.get(laneOf[id]));

  const laneCount = Math.max(1, lanesEnd.length || 1);
...
/** ======================== GLAVNA KOMPONENTA ======================== */
// Inner component that uses the HoverContext
function PlannerGanttV2() {
  const { project } = useProjectStore();
  const [projectService] = useState(() => new ProjectDataService());
  const { hoverMeta, setHoverMeta } = useHover();
  ...
  useEffect(() => {
    ...
    const initializeData = async () => {
      try {
        ...
        // Try to load real JSON data first from local directory
        let projectData = null;
        try {
          // Try to fetch the new 2-project file from local component directory
          const response = await fetch('/src/components/tabs/PlannerGanttV2/all_projects_two_projects.json');
          if (response.ok) {
            projectData = await response.json();
            console.log('PlannerGanttV2: Loaded 2-project data from local JSON:', projectData);
          } else {
            throw new Error('2-project JSON not found, trying single project');
          }
        } catch (err) {
          console.log('PlannerGanttV2: Could not load 2-project file, trying single project JSON');
          // Fallback to single project file
          try {
            const response = await fetch('/src/components/tabs/PlannerGanttV2/project_from_troskovnik_planning.json');
            if (response.ok) {
              projectData = await response.json();
              console.log('PlannerGanttV2: Loaded single project from local JSON:', projectData);
            } else {
              // Final fallback to public directory
              const response2 = await fetch('/project_from_troskovnik_planning.json');
              if (response2.ok) {
                projectData = await response2.json();
                console.log('PlannerGanttV2: Loaded project data from public JSON:', projectData);
              }
            }
          } catch (err2) {
            console.log('PlannerGanttV2: Could not load any JSON file, trying service data');
          }
        }

        // Fallback to ProjectDataService if JSON not available
        if (!projectData) {
          projectData = await projectService.loadAllProjects();
        }
        
        // If still no data exists, create from current Zustand project
    ...
    if (isAllProjectsView) {
        ...
        if (project.positions) {
          project.positions.forEach(position => {
            ...
            const positionTasks = (position.processes || []).map(process => ({
              ...
              ownerId: process.owner?.id || 'unassigned',
              ownerName: process.owner?.name || 'Nedodijeljeno',
              ownerEmail: process.owner?.email || null,
              notes: process.notes,
              urgency: 'normal',
              meta: {
                entity: 'process',
            ...
            // Add checklist tasks as 1-day bars
            const checklistBars = (position.tasks || []).flatMap(t => {
              if (!t.due) return [];
              return [{
                id: `${project.id}-${position.id}-${t.id}`,
                naziv: t.title,
                pozicija: position.id,
                proces: 'općenito',
                status: t.status?.toLowerCase() === 'done' ? 'završeno' : 'u tijeku',
                start: t.due,
                end: addDays(t.due, 1),
                projectId: project.id,
                projectName: project.name,
                owner: t.assignee?.name,
                ownerId: t.assignee?.id || 'unassigned',
                ownerName: t.assignee?.name || 'Nedodijeljeno',
                ownerEmail: t.assignee?.email || null,
                type: 'checklist',
                meta: {
                  entity: 'position_task',
                  projectId: project.id,
                  positionId: position.id,
                  taskId: t.id
                }
            ...
            // Add process subtasks as 1-day bars
            const subtaskBars = [];
            (position.processes || []).forEach(process => {
              (process.subtasks || []).forEach(subtask => {
                if (!subtask.due) return;
                subtaskBars.push({
                  id: `${project.id}-${position.id}-${process.name}-${subtask.id}`,
                  naziv: subtask.title,
                  pozicija: position.id,
                  proces: 'subtask',
                  status: subtask.status?.toLowerCase() === 'done' ? 'završeno' : 
                         subtask.status?.toLowerCase() === 'doing' ? 'u tijeku' : 'čeka',
                  start: subtask.due,
                  end: addDays(subtask.due, 1),
                  projectId: project.id,
                  projectName: project.name,
                  owner: subtask.assignee?.name,
                  ownerId: subtask.assignee?.id || 'unassigned',
                  ownerName: subtask.assignee?.name || 'Nedodijeljeno',
                  ownerEmail: subtask.assignee?.email || null,
                  type: 'subtask',
            ...
            allTasks.push(...positionTasks, ...checklistBars, ...subtaskBars);
      ...
      if (project.positions) {
        project.positions.forEach(position => {
          ...
          projectTasks.push(...positionTasks, ...checklistBars, ...subtaskBars);
        });
      }
  ...
  // Selector function for hover data based on meta object
  const selectHoverData = useCallback((meta) => {
    if (!meta || !data) return null;

    try {
      switch (meta.entity) {
        case 'process':
          const project = data.projects?.find(p => p.id === meta.projectId);
          const position = project?.positions?.find(pos => pos.id === meta.positionId);
          const process = position?.processes?.find(proc => proc.name === meta.processName);
          return {
            entity: 'process',
            project: project,
            position: position,
        ...
        case 'position_task':
          const taskProject = data.projects?.find(p => p.id === meta.projectId);
          const taskPosition = taskProject?.positions?.find(pos => pos.id === meta.positionId);
          const task = taskPosition?.tasks?.find(t => t.id === meta.taskId);
          return {
            entity: 'position_task',
            project: taskProject,
        ...
        case 'subtask':
          const subtaskProject = data.projects?.find(p => p.id === meta.projectId);
          const subtaskPosition = subtaskProject?.positions?.find(pos => pos.id === meta.positionId);
          const subtaskProcess = subtaskPosition?.processes?.find(proc => proc.name === meta.processName);
          const subtask = subtaskProcess?.subtasks?.find(st => st.id === meta.subtaskId);
          return {
            entity: 'subtask',
            project: subtaskProject,
            position: subtaskPosition,
            process: subtaskProcess,
            subtask: subtask,
            data: {
              id: subtask?.id,
              title: subtask?.title,
        ...
        default:
          console.warn('Unknown hover entity type:', meta.entity);
          return null;
      }
    } catch (error) {
      console.error('Error selecting hover data:', error);
      return null;
    }
  }, [data]);

  const [viewMode, setViewMode] = useState('zaposlenici'); // Default to employee view for testing
  ...
  // Helper function to map JSON task titles to process IDs
  const mapTaskToProcess = (taskTitle) => {
    const title = taskTitle.toLowerCase();
    if (title.includes('rokove') || title.includes('ugradnja') || title.includes('dizalica') || title.includes('pristup')) {
      return 'ugradnja';
    }
    if (title.includes('materijal') || title.includes('nabav') || title.includes('dostupnost')) {
      return 'nabava';
    }
    if (title.includes('sigurnost') || title.includes('zaštit') || title.includes('plan')) {
      return 'teh_priprema';
    }
    if (title.includes('dizajn') || title.includes('nacrt')) {
      return 'dizajn';
    }
    if (title.includes('prodaj') || title.includes('ponud')) {
      return 'prodaja';
    }
    return 'općenito'; // Default fallback
  };

  const filteredTasks = useMemo(() => {
    // Get existing tasks from currentViewData
    const existingTasks = (currentViewData?.tasks || []).filter(t => {
    ...
    // Get tasks from JSON positions data and convert to Gantt task format
    const positionTasks = [];
    if (data?.projects) {
      const activeProject = data.projects.find(p => 
        isAllProjectsView ? true : p.id === activeProjectId
      );
      
      if (activeProject?.positions) {
        activeProject.positions.forEach(position => {
          if (position.tasks && Array.isArray(position.tasks)) {
            position.tasks.forEach(task => {
              // Map task to appropriate process
              const processId = mapTaskToProcess(task.title || '');
              
              // Convert JSON task to Gantt task format
              const ganttTask = {
                id: `pos-${task.id}`, // Prefix to avoid ID conflicts
                naziv: task.title || 'Untitled Task',
                pozicija: position.id,
                projectName: activeProject.name,
                projectId: activeProject.id,
                proces: processId,
                status: task.status === 'completed' ? 'završeno' : 
                       task.status === 'in_progress' ? 'u tijeku' : 'čeka',
                urgency: task.due ? 'high' : 'normal', // Tasks with due dates are high priority
                start: task.due ? new Date(new Date(task.due).getTime() - 7 * 24 * 60 * 60 * 1000).toISOString().split('T')[0] : null, // 7 days before due date
                end: task.due || null,
                assignedTo: task.assignee?.name || 'Unassigned',
                ownerId: task.assignee?.id || 'unassigned',
                ownerName: task.assignee?.name || 'Unassigned',
                opis: `Position task: ${task.title}`,
                komentari: [],
                prilozi: [],
                isPositionTask: true // Flag to identify these as position tasks
              };
              
              // Apply same filters as existing tasks
              if (processFilters.has(ganttTask.proces)) {
                if (!searchText || (`${ganttTask.pozicija} ${ganttTask.naziv} ${ganttTask.projectName || ''}`.toLowerCase().includes(searchText.toLowerCase()))) {
                  positionTasks.push(ganttTask);
                }
              }
            });
          }
    ...
    console.log('📊 Gantt Tasks Summary:');
    console.log('- Existing tasks:', existingTasks.length);
    console.log('- Position tasks:', positionTasks.length);
    console.log('- Total tasks for Gantt:', existingTasks.length + positionTasks.length);
    if (positionTasks.length > 0) {
      console.log('- Position task processes:', positionTasks.map(t => `${t.naziv} → ${t.proces}`));
    }

    return [...existingTasks, ...positionTasks];
  }, [currentViewData?.tasks, currentViewData?.subtasksByPosition, processFilters, searchText, indicatorFilters, data, activeProjectId, isAllProjectsView]);

  const { groups, flatRows, totalRows } = useMemo(() => {
    // 1) Group tasks
    const grouped = new Map();
    filteredTasks.forEach(task => {
      let key;
      if (viewMode === 'pozicije') key = task.pozicija;
      else if (viewMode === 'procesi') key = task.proces;
      else key = task.ownerId || 'unassigned';   // === zaposlenici
      if (!grouped.has(key)) grouped.set(key, []);
      grouped.get(key).push(task);
    });

    // 2) Sort within group by date
    ...
    // 3) Prepare groupsArr (display names)
    const groupsArr = Array.from(grouped.entries()).map(([name, tasks]) => ({
      name,
      displayName: viewMode === 'procesi'
        ? (PROCESI.find(p => p.id === name)?.naziv || name)
        : viewMode === 'zaposlenici'
          ? (tasks[0]?.ownerName || 'Nedodijeljeno')
          : name,
      tasks
    }));

    // 4) Flatten into rows (+ lanes)
    let nextRow = 0;
    const rows = [];

    for (const g of groupsArr) {
      // Header row
      rows.push({ type: 'header', key: `h-${g.name}`, group: g, rowIndex: nextRow++ });

      if (viewMode !== 'zaposlenici') {
        // Old behavior: one row per task
        g.tasks.forEach(t => rows.push({ type: 'task', key: t.id, task: t, rowIndex: nextRow++ }));
        continue;
      }

      // Zaposlenici: lanes
      const { laneOf, laneCount } = assignLanesSorted(g.tasks);

      // Tasks positioned in lanes
      const base = nextRow;
      g.tasks.forEach(t => {
        rows.push({ 
          type: 'task', 
          key: t.id, 
          task: t, 
          rowIndex: base + laneOf[t.id],
          isInLane: true,
          laneNumber: laneOf[t.id]
        });
      });
      nextRow += laneCount;
    }

    return { groups: groupsArr, flatRows: rows, totalRows: nextRow };
  }, [filteredTasks, viewMode]);

  const chartHeight = totalRows * ROW_H;
  ...
  const visibleRowEnd = Math.min(totalRows, Math.ceil((viewport.top + viewport.height) / ROW_H) + OVERSCAN_ROWS);
    ...
    return React.memo(function TaskBarInner({ task, rowIndex }) {
      ...
      return (
        <>
          ...
          >
            <div 
              className={`h-full rounded-lg border-2 overflow-hidden ${isSelected ?'ring-2 ring-blue-500 ring-offset-1':''} ${settings.highlightOverdue && task.status === 'kasni' ? 'animate-pulse' : ''} ${task.proces === 'općenito' && !useProjectColors ? 'rainbow-gradient' : ''} ${task.type === 'subtask' && !useProjectColors ? 'subtask-gradient' : ''} ${task.isPositionTask && !useProjectColors ? 'position-task-gradient' : ''}`} 
              style={{
                ...
                background: task.proces === 'općenito' && !useProjectColors ? 
                           ...
                           task.type === 'subtask' && !useProjectColors ?
                           'linear-gradient(135deg, #10b981 0%, #06b6d4 25%, #3b82f6 50%, #8b5cf6 75%, #ec4899 100%)' :
                           task.isPositionTask && !useProjectColors ?
                           'linear-gradient(135deg, #f59e0b 0%, #f97316 25%, #dc2626 50%, #b91c1c 75%, #7c2d12 100%)' :
            ...
            >
              ...
              <div className="relative h-full flex items-center px-2">
                ...
                />
                <span className={`text-xs font-medium truncate ${(task.proces === 'općenito' || task.type === 'subtask' || task.isPositionTask) && !useProjectColors ? 'text-white' : 'text-slate-700'}`}>
  ...
  const handleTaskHover = useCallback((task, element) => {
    console.group('🐛 DEBUG: handleTaskHover');
    console.log('1. Task input:', task);
    console.log('2. Task.pozicija:', task.pozicija);
    console.log('3. activeProjectId:', activeProjectId);
    console.log('4. isAllProjectsView:', isAllProjectsView);
    console.log('5. currentViewData:', currentViewData);
    console.log('5a. currentViewData.projects:', currentViewData?.projects);
    console.log('5b. currentViewData.projects IDs:', currentViewData?.projects?.map(p => p.id));
    console.log('5c. Direct data:', data);
    console.log('5d. data.projects:', data?.projects);
    console.log('5e. data.projects IDs:', data?.projects?.map(p => p.id));
    ...
    // Create meta object from real project data - use data.projects instead of currentViewData
    const activeProject = data?.projects?.find(p => 
      isAllProjectsView ? true : p.id === activeProjectId
    );
    console.log('6. activeProjectId we\'re looking for:', activeProjectId);
    console.log('6a. Found activeProject:', activeProject);
    console.log('7. Available positions:', activeProject?.positions?.map(p => ({ id: p.id, title: p.title })));
    
    const realPosition = activeProject?.positions?.find(pos => pos.id === task.pozicija);
    console.log('8. Found realPosition:', realPosition);
    console.log('9. realPosition.tasks:', realPosition?.tasks);
    
    if (realPosition) {
      const mappedTasks = realPosition.tasks?.map(t => ({
        id: t.id,
        title: t.title,
      ...
      console.log('10. Mapped tasks for hover:', mappedTasks);
      
      const hoverMeta = {
        position: {
          id: realPosition.id,
          title: realPosition.title,
          processData: {
            comments: realPosition.comments?.map(c => ({
              id: c.id,
              text: c.text,
              author: c.author?.name || "Unknown",
              date: new Date(c.date).toISOString().split('T')[0]
            })) || [],
            documents: realPosition.documents || [],
            tasks: mappedTasks
          },
          pieces: [{
            id: 1,
            ...
            montaza: realPosition.processes?.reduce((acc, proc) => {
      ...
      console.log('11. Final hoverMeta.position.processData.tasks:', hoverMeta.position.processData.tasks);
      setHoverMeta(hoverMeta);
    } else {
      console.log('❌ No realPosition found, using fallback');
      if (task.meta) {
        setHoverMeta(task.meta);
      } else {
        console.log('❌ No task.meta fallback either');
      }
    }
    console.groupEnd();
  }, [setHoverMeta, currentViewData, activeProjectId, isAllProjectsView, data]);
    ...
    <div className="h-full flex flex-col bg-slate-50" ref={ganttRef}>
      <div className="h-14 bg-white border-b px-4 flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <button
              onClick={() => setViewMode('zaposlenici')}
              className={`p-2 rounded-lg text-sm transition-colors flex items-center gap-1 ${
                viewMode === 'zaposlenici' ? 'bg-blue-600 text-white'
                : 'bg-white border border-slate-300 text-slate-700 hover:bg-slate-50'
              }`}
              title="Po zaposlenicima"
            >
      ...
      <div className="flex-1 flex overflow-hidden">
        <div className="w-80 bg-white border-r flex flex-col">
          <div className="h-20 border-b px-4 flex items-end pb-2">
            <span className="text-sm font-medium text-slate-600">
              {viewMode === 'pozicije' ? 'Pozicije' : viewMode === 'procesi' ? 'Procesi' : 'Zaposlenici'}
            </span>
          </div>
          <div 
            className="gantt-left-panel flex-1 overflow-y-auto" 
            onScroll={(e) => {
              // Keep left panel synchronized with right panel scroll
              const rightPanel = document.querySelector('.gantt-right-panel');
              if (rightPanel && !rightPanel.dataset.scrolling) {
                rightPanel.dataset.scrolling = 'true';
                rightPanel.scrollTop = e.target.scrollTop;
                setTimeout(() => delete rightPanel.dataset.scrolling, 50);
              }
            }}
          >
            {/* Simple approach: render all rows but with proper lane spacing */}
            {Array.from({ length: totalRows }).map((_, rowIdx) => {
              const rowContent = flatRows.find(r => r.rowIndex === rowIdx);
              
              if (rowContent?.type === 'header') {
                const row = rowContent;
                return (
                  <div 
                    key={row.key} 
                    className="h-9 px-4 bg-slate-50 border-b flex items-center cursor-pointer z-10" 
                    style={{ height: ROW_H }}
                  >
                    ...
                    {viewMode !== 'zaposlenici' && (
                      <button onClick={() => setShowPositionHistory(row.group.name)} className="ml-auto p-1 hover:bg-slate-200 rounded" title="Povijest pozicije">
                        <History className="w-3 h-3 text-slate-600" />
                      </button>
                    )}
                    <span className="ml-2 text-xs text-slate-500">{row.group.tasks.length}</span>
                  </div>
                );
              }
              
              if (rowContent?.type === 'task') {
                const row = rowContent;
                const t = row.task;
                const proces = PROCESI.find(p => p.id === t.proces);
                const missingDates = !t.start || !t.end;
                const urgency = URGENCY_LEVELS[t.urgency || 'normal'];
                return (
                  <div
                    key={row.key}
                    className={`h-9 px-6 border-b flex items-center hover:bg-slate-50 cursor-pointer ${selectedTask?.id === t.id ? 'bg-blue-50' : ''} ${missingDates && settings.highlightNoDates ? 'bg-red-50' : ''}`}
                    style={{ height: ROW_H }}
                    onClick={() => setSelectedTask(t)}
                  >
                    {proces && <proces.ikona className="w-3 h-3 mr-2 flex-shrink-0" style={{ color: proces.id === 'općenito' ? '#764ba2' : proces.boja }} />}
                    <span className={`text-xs truncate flex-1 ${missingDates ? 'text-red-600 font-bold' : ''}`}>
                      {viewMode === 'pozicije' ? t.naziv
                       : viewMode === 'procesi' ? t.pozicija
                       : `${t.projectName || ''} — ${t.naziv}`}
                    </span>
                    {isAllProjectsView && <div className="w-3 h-3 rounded-full mr-2" style={{ backgroundColor: generateProjectColor(t.projectName || '') }} title={t.projectName} />}
                    {t.urgency && t.urgency !== 'normal' && <urgency.icon className="w-3 h-3 mr-1" style={{ color: urgency.bg }} />}
                    <span className="w-2 h-2 rounded-full ml-2 flex-shrink-0" style={{ backgroundColor: STATUSI[t.status]?.bg }} />
                  </div>
                );
              }
              
              // Empty lane row - transparent placeholder to maintain spacing
              return (
                <div 
                  key={`empty-${rowIdx}`} 
                  className="h-9 border-b bg-white opacity-20" 
                  style={{ height: ROW_H }}
                />
              );
            })}
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div 
            ref={chartRef} 
            className="gantt-right-panel flex-1 overflow-auto bg-white relative" 
            style={{ scrollBehavior: 'smooth' }}
            onScroll={(e) => {
              // Update viewport for floating calendar
              scrollHandler();
              
              // Keep right panel synchronized with left panel scroll
              const leftPanel = document.querySelector('.gantt-left-panel');
              if (leftPanel && !leftPanel.dataset.scrolling) {
                leftPanel.dataset.scrolling = 'true';
                leftPanel.scrollTop = e.target.scrollTop;
                setTimeout(() => delete leftPanel.dataset.scrolling, 50);
              }
            }}
          >
            {/* Floating Calendar Header - positioned inside chart container */}
            {viewport.top > 100 && (
              <div 
                className="absolute bg-white border-b shadow-lg z-30 overflow-hidden"
                style={{
                  top: viewport.top,  // Position at current scroll top
                  left: 0,
                  width: `${(visibleDayEnd-visibleDayStart) * dayWidth}px`, 
                  height: '80px',
                  transform: `translateX(${visibleDayStart * dayWidth}px)`  // Follow horizontal scroll
                }}
              >
                    ...
                    {Array.from({ length: visibleDayEnd - visibleDayStart }).map((_, idx) => { 
                      ...
                      return (
                        ...
                        >
                          ...
                          {zoom === 'week' && date.getDay() === 1 && (
                            <div className="text-center">
                              <div className="font-medium">W{Math.ceil(date.getDate()/7)}</div>
                              <div className="text-[10px] text-slate-500">
                                {date.toLocaleDateString('hr-HR', { month: 'short' })}
                              </div>
                            </div>
            ...
            <div className="relative" style={{ width: `${totalDays * dayWidth}px`, height: `${chartHeight}px` }}>
              ...
              {Array.from({ length: Math.max(0, visibleRowEnd - visibleRowStart) }).map((_, idx) => {
                const absoluteRowIdx = visibleRowStart + idx;
                if (absoluteRowIdx >= totalRows) return null;

                // Find if this row has content (header or task)
                const rowContent = flatRows.find(r => r.rowIndex === absoluteRowIdx);
                const isHeader = rowContent?.type === 'header';
                const top = absoluteRowIdx * ROW_H;

                return (
                  <div
                    key={`bg-${absoluteRowIdx}`}
                    className={`absolute left-0 right-0 border-b ${
                      isHeader ? 'bg-slate-50' : 'bg-white'
                    }`}
                    style={{ top, height: ROW_H }}
                  />
                );
              })}
              {/* Render all task bars - no virtualization for task bars to ensure they show on scroll */}
              {flatRows.filter(row => row.type === 'task').map(row => {
      ...
      <AnimatePresence>
        {memoizedHoverTask && hoverLevel > 0 && !dragState && (
          <TaskHoverCardRedesign
            ...
            projectService={projectService}
            projectId={activeProjectId}
            selectHoverData={selectHoverData}
...
// Main wrapper component that provides HoverContext
