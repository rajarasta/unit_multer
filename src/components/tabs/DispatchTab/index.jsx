...
import LargeGridCard from './LargeGridCard';
...
const DispatchTab = () => {
  ...
  const getStatusColor = (status) => {
    switch (status) {
      case 'Done':
      case 'završeno': return 'text-green-600 bg-green-50';
      case 'In Progress':
      case 'u_radu':
      ...
      case 'Blocked':
      case 'problem': return 'text-red-600 bg-red-50';
    ...
    <div className="h-full flex flex-col bg-gray-50">
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 overflow-y-auto p-6">
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4" style={{ gridAutoRows: 'minmax(200px, auto)' }}>
            <AnimatePresence>
              {filteredPositions.map((position, index) => {
                // Auto-detect large grid positions (demo data or title-based)
                if (position.isSpecialCard && position.type === 'large_grid_special') {
                  return (
                    <LargeGridCard
                      key={position.id}
                      position={position}
                ...
                // Auto-detect large grid by title keywords 
                if (position.title && (
                    position.title.toLowerCase().includes('veliki') ||
                    position.title.toLowerCase().includes('20x2') ||
                    position.title.toLowerCase().includes('masivni') ||
                    (position.title.toLowerCase().includes('fasad') && position.qty > 20)
                  )) {
                  return (
                    <LargeGridCard
                      key={position.id}
                ...
                // Auto-detect facade positions and render as special cards  
                if ((position.isSpecialCard && position.type === 'facade_special') || 
                    (position.title && position.title.toLowerCase().includes('fasad') && position.qty <= 20)) {
                  return (
                    <FacadeCard
                      key={position.id}
                      position={position}
