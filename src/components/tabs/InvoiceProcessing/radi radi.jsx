...
// Enhanced System Prompt for Spatial Coordinate Analysis
...
Primiti ćeš JSON objekt koji sadrži:
1. **elements**: Niz objekata, svaki s koordinatama i tekstom:
   - text: Tekst elementa
   - x, y: Pozicija elementa na stranici
   - width, height: Dimenzije elementa
   - page: Broj stranice (ako PDF)

2. **spatialText**: Predobrađeni tekst rekonstruiran iz koordinata (kao rezerva)

### KAKO ANALIZIRATI KOORDINATE:
1. **Prostorni Layout**: Koristi X,Y koordinate za razumijevanje strukture
   - Elementi s sličnim Y koordinatama su u istom retku
   - Elementi s velikim X razmakom su u različitim stupcima
   - Manji Y = viši na stranici, veći Y = niži na stranici

2. **Tablične Strukture**: 
   - Identificiraj zaglavlja tabela po Y poziciji
   - Grupiraj elementi s istim X koordinatama kao stupce
   - Spoji elemente koji čine jednu stavku (isti ili sličan Y)

3. **Sekcije Dokumenta**:
   - Header (mali Y): Naziv tvrtke, broj dokumenta
   - Sredina: Stavke/tablice
   - Footer (veliki Y): Sažeci, potpisi
...
- OBVEZNO: Vrati ISKLJUČIVO JSON objekt, bez dodatnog teksta, objašnjenja ili komentara
- JSON mora počinjati s { i završavati s }
- Koristi prostorne odnose za precizno parsiranje tabela
`;
