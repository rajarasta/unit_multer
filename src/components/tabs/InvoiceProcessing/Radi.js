...
const LM_STUDIO_CONFIG = {
  endpoint: 'http://192.168.245.136:1234/v1/chat/completions',
  model: 'openai/gpt-oss-20b',
  temperature: 0.1, // Very low temperature for deterministic results
  max_tokens: 8000,
};

// LLM Input Mode Configuration
const LLM_INPUT_MODE = 'page_images'; 
// 'page_images' | 'pdf' | 'none'
// page_images = render first N pages to JPEGs
// pdf = attach the raw PDF (data:application/pdf;base64,...) - may or may not be supported by your local build

const LLM_PAGE_LIMIT = 4;   // keep payloads sane, bump if needed
const LLM_IMAGE_SCALE = 1.5; // 1.0-2.0. Higher = crisper images, larger payload.

// Enhanced System Prompt with Multimodal Rules
const LLM_SYSTEM_PROMPT = `
Ti si AI asistent specijaliziran za analizu hrvatskih poslovnih dokumenata (ponude, računi, otpremnice, primke, međuskladišnice).

## Ulazi koje možeš dobiti
- **PDF ili slike stranica** (primarni izvor istine ako postoje).
- **elements** (X,Y koordinate, širina, visina, stranica, tekst).
- **spatialText** (rekonstrukcija teksta iz koordinata, rezervni izvor).

## Pravila analize
1) Ako je PDF ili slike stranica priložen: vizualno interpretiraj raspored (tablice, zaglavlja, stupci, fusnote). 
2) Zatim koristi **elements** i **spatialText** za validaciju brojčanih vrijednosti i precizno spajanje polja.
3) Tablice: prepoznaj redove po sličnom Y, stupce po X, spoji elemente u jedinstvene stavke (opis može ići u više linija).
4) Prepoznaj sekcije:
   - Header (mali Y): dobavljač, broj dokumenta, datum
   - Sredina: stavke/tablice
   - Footer (veliki Y): sažeci, PDV, ukupno, potpisi
5) Hrvatski formati:
   - Brojevi "1.234,56" → JSON: 1234.56
   - Datumi "15.01.2024." → ISO: 2024-01-15
6) Ako nešto nedostaje, vrati null umjesto nagađanja.

## IZLAZ (obavezno SAMO JSON)
{
  "documentType": "quote|invoice|delivery|receipt|transfer|other",
  "documentNumber": "string",
  "date": "YYYY-MM-DD|null",
  "dueDate": "YYYY-MM-DD|null",
  "currency": "EUR|BAM|HRK|null",
  "supplier": { "name": "string|null", "address": "string|null", "oib": "string|null", "iban": "string|null" },
  "buyer": { "name": "string|null", "address": "string|null", "oib": "string|null" },
  "items": [
    { "position": "number|null", "code": "string|null", "description": "string|null", "quantity": "number|null", "unit": "string|null", "unitPrice": "number|null", "discountPercent": "number|null", "totalPrice": "number|null" }
  ],
  "totals": { "subtotal": "number|null", "vatAmount": "number|null", "totalAmount": "number|null" }
}

Vrati ISKLJUČIVO JSON objekt, bez dodatnog teksta. JSON mora počinjati s { i završavati s }.
`;
...
// Helper: read a File as data URL (works for PDF)
const fileToDataURL = (file) => new Promise((resolve, reject) => {
  const r = new FileReader();
  r.onload = () => resolve(r.result); // e.g. "data:application/pdf;base64,...."
  r.onerror = reject;
  r.readAsDataURL(file);
});

// Helper: render first N PDF pages into JPEG data URLs
async function renderPdfToImagesDataUrls(file, pageLimit = 3, scale = 1.25) {
  const arrayBuffer = await file.arrayBuffer();
  const pdf = await getDocument({ data: arrayBuffer }).promise;

  const urls = [];
  const maxPages = Math.min(pageLimit, pdf.numPages);
  for (let p = 1; p <= maxPages; p++) {
    ...
    // JPEG is usually much smaller than PNG; quality 0.85 is a good balance
    const dataUrl = canvas.toDataURL('image/jpeg', 0.85);
    urls.push({ page: p, dataUrl });
  }
  return { images: urls, pageCount: pdf.numPages };
}
...
export default function InvoiceProcesser() {
  ...
  // Analyze with LLM using spatial coordinate data
  const analyzeWithLLM = useCallback(async (extractedData) => {
    ...
    try {
      console.log('🚀 Sending request to:', LM_STUDIO_CONFIG.endpoint);
      console.log('🤖 Model:', LM_STUDIO_CONFIG.model);
      console.log('📊 Elements:', spatialData.elements.length);
      ...
      if (!response.ok) {
        ...
        console.error('🔧 Model being used:', LM_STUDIO_CONFIG.model);
        console.error('🌐 Endpoint:', LM_STUDIO_CONFIG.endpoint);
      ...
      try {
        ...
        // Very high confidence due to direct coordinate analysis
...
function DebugPanel({ document, llmStatus }) {
    const [viewMode, setViewMode] = useState('spatial'); // spatial, raw, elements, tables, system
      ...
      <div className="mt-6 bg-gray-900 text-green-400 rounded-xl p-6 font-mono shadow-xl">
        <div className="flex justify-between items-center mb-4">
            ...
            <div className="flex gap-2 flex-wrap">
                <button onClick={() => setViewMode('spatial')} className={`px-3 py-1 rounded text-sm ${viewMode === 'spatial' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300'}`}>LLM Input (JSON)</button>
                <button onClick={() => setViewMode('raw')} className={`px-3 py-1 rounded text-sm ${viewMode === 'raw' ? 'bg-blue-600 text-white' : 'bg-gray-700 text-gray-300'}`}>Rekonstruirani Tekst</button>
        ...
        <div className="bg-gray-800 p-4 rounded-lg overflow-auto max-h-[60vh]">
            {viewMode === 'spatial' && (
                <>
                <p className='text-sm mb-2 text-blue-400'>Prikaz JSON podataka koji se šalju LLM-u s koordinatama i metapodacima.</p>
